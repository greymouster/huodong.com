...
﻿ <div class="home-left">
    ...
    <div class="account-info modify-limit add-person">
        ...
        <form>
            ...
            <?php if(is_array($arr1)): foreach($arr1 as $key=>$v1): ?><dl>
                <dt><h3><?php echo ($v1["auth_name"]); ?>：</h3></dt>
                <dd><input class="checkAll" type="checkbox" name="checkbox" />全选</dd>
                <?php if(is_array($arr)): foreach($arr as $key=>$vv): if($v1['id'] == $vv['parent_id']): ?><dd><input class="checkItem" type="checkbox" name="act_list[]" value="<?php echo ($vv["id"]); ?>" /><?php echo ($vv["auth_name"]); ?></dd><?php endif; endforeach; endif; ?>
...
<script>
    $(function(){
        ...
        $('input[type="text"]').blur(function(){
            ...
            $.ajax({
                url: '/index.php/Admin/Auth/checkUser',
                data: {username:val},
                type: 'POST',
                dataType: 'json',
