...
class ActmessageModel extends Model {
    ...
    public function getSearchData($where) {
        ...
        $Page = new NewPage($count, 4);
        $show = $Page->fpage(array(4, 5, 6));
    ...
    //获取当前的状态
    public function getCurrentStatus($data,$act_date,$act_time) {
        switch ($data) {
            case 1:
                return '未发布';
                break;
...
//        }
    ...
    //时间戳转换
    public function dataTrans($data) {
        $data['act_start_date'] = date('Y-m-d', $data['act_start_date']);
        $data['act_end_date'] = date('Y-m-d', $data['act_end_date']);
        $data['act_start_time'] = date('H:i', $data['act_start_time']);
        $data['act_end_time'] = date('H:i', $data['act_end_time']);
        $data['act_date'] = date('Y-m-d', $data['act_date']);
        $data['act_time'] = date('H:i', $data['act_time']);
        $data['act_week'] = explode(',', $data['act_week']);
        return $data;
    }
